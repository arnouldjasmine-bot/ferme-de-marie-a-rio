...
export async function PATCH(
  ...
  try {
    ...
    const commandes: Commande[] = JSON.parse(raw)
    const idx = commandes.findIndex(c => c.id === id)
    if (idx === -1) return NextResponse.json({ ok: false, error: 'Commande introuvable' }, { status: 404 })
    ...
    if (statut === 'confirmee' && ancienStatut !== 'confirmee' && process.env.RESEND_API_KEY) {
      ...
      try {
        ...
        const lignesArticles = cmd.articles.map(a =>
          `<tr>
...
<body style="margin:0;padding:0;background:#F5F2E9;font-family:Georgia,serif;">
  <div style="max-width:580px;margin:32px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 16px rgba(74,93,78,0.10);">
    ...
    <div style="background:#4A5D4E;padding:32px 24px;text-align:center;">
      ...
      <h1 style="margin:8px 0 0;font-size:26px;color:#fff;font-weight:normal;">Votre commande est confirmée ! 🎉</h1>
    </div>

    <div style="padding:32px 24px;">
      <p style="margin:0 0 8px;color:#4A5D4E;font-size:15px;">Bonjour <strong>${cmd.prenom}</strong>,</p>
      <p style="margin:0 0 24px;color:#5a5a4a;font-size:14px;line-height:1.6;">
        Bonne nouvelle ! Nous avons confirmé votre commande et nous allons préparer vos produits frais avec soin.
      ...
      <div style="background:#f8f6f0;border-radius:12px;padding:20px;margin-bottom:24px;">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:14px;">
          ...
          <tfoot>
            <tr>
              <td colspan="2" style="padding:12px;font-weight:bold;color:#4A5D4E;font-size:15px;">Total</td>
              <td style="padding:12px;font-weight:bold;color:#4A5D4E;font-size:15px;text-align:right;">R$ ${cmd.total.toFixed(2)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
      ...
      <div style="border-left:3px solid #93A27D;padding:12px 16px;background:#f4f7f4;border-radius:0 8px 8px 0;margin-bottom:24px;">
        ...
        <p style="margin:0;font-size:14px;color:#3a3a2a;">${cmd.adresse}</p>
      </div>

      <p style="margin:0;font-size:13px;color:#7a7a6a;line-height:1.6;">
        Des questions ? Contactez-nous sur WhatsApp ou répondez à cet email.
      </p>
    </div>
